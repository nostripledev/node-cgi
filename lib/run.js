...
module.exports = async (src, glob) => {
  ...
  let code = '';
  const txts = [];
  const app_code = async (x) => {
    code += `${x.trim()};\n`;
  };
  const app_eval = async (x) => {
    app_code(`write(${x})`);
  };
  const app_text = async (x) => {
    app_eval(`${TXTS_ID}[${txts.length}]`);
    txts.push(x);
  };
  ...
  while (true) {
    ...
    const type = match?.[1];
    if (type === 'php++' || !type) {
      let php_code = inner.slice(match[0].length);
      php_code = php_code.replace(/\$([a-zA-Z_]\w*)/g, 'var_$1');
      app_code(php_code);
    } else if (type === '=') {
      let php_code = inner.slice(match[0].length);
      php_code = php_code.replace(/\$([a-zA-Z_]\w*)/g, 'var_$1');
